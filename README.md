...
## Analysis-results: 
The following image shows the scraped data stored in Mongo db compass. This challenge collected full images of each link and title and stored in Mongo as a data dictionary.
![This image shows the scraped data stored in Mongodb compass ](mongodb_stored_mars_data.PNG)

The following image shows the index.html where the Mongodb data is read and presented to the user in html format. It has a button to scrape new data as well. This image displays the Mars hemisphere in table two on each row with thumbnail.
